...
class PDFInfo:
    def __init__(self, file):
        ...
        self.trim_width = (self.trim_size[2] - self.trim_size[0]) / 72
        self.trim_height = (self.trim_size[3] - self.trim_size[1]) / 72
...
if __name__ == '__main__':
    ...
    with open(f'{directory_name}.csv', 'w', encoding='utf-8') as f:
        ...
        for pdf in file_list:
            ...
            record = f'"{pdf.name}",' \
                     f'"{info.page_count}",' \
                     f'"{info.trim_width:0.2f} x {info.trim_height:0.2f}",' \
